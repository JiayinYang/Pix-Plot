...
// Count of 32px and 32px atlas files to fetch
var atlasCounts = {
  '32px': null,
  '64px': null
}
...
// Create a store for the 32px and 32px atlas materials
var materials = {
  32: [],
  64: []
}
...
//textureLoader.miniFilter = THREE.NearestFilter;
/**
...
function getCamera() {
  ...
  var camera = new THREE.PerspectiveCamera(75, aspectRatio, 0.0001, 50000);
  camera.position.set(500, -10000, 52000);
  return camera;
}
...
function onProgress(atlasIndex, xhr) {
  ...
  progress = sum / (img_file.length + 1);
  loader.innerHTML = parseInt(progress * 100) + '%';
  if (sum === 1) startIfReady();
  if (progress === 1) {
...
function updateVertices(geometry, img) {
  geometry.vertices.push(
    ...
    new THREE.Vector3(
      img.pos.x + (img.width * 4.5),
      img.pos.y,
      img.pos.z
    ),
    new THREE.Vector3(
      img.pos.x + (img.width * 4.5),
      img.pos.y + (img.height * 4.5),
      img.pos.z
    ),
    new THREE.Vector3(
      img.pos.x,
...
function loadLargeAtlasFiles() {
  ...
  for (var i = 0; i < img_file.length; i++) {
    var url = dataUrl + 'thumbs/128px/' + img_file[i];
    textureLoader.load(url, handleLargeTexture.bind(null, i), onProgress.bind(null, i + 1));
  }
}
...
function onMouseup(event) {
  ...
  window.open('graph.html', '_blank', 'width=600,height=600,menubar=no,toolbar=no, location=0, status=no,scrollbars=yes,replace=false')
  ...
  //console.log(imageDataKeys[imageIndex]); // image
  // Store the image name in the url hash for reference
  window.location.hash = imageDataKeys[imageIndex];
  document.getElementById("newele").value = imageDataKeys[imageIndex];
...
function update(clusters) {
  get('output/plot_data.json', function(data1) {
    var data1 = JSON.parse(data1);
    var cluster = [];
    for (var i = 0; i < clusters.length; i++) {
      cluster.push(data1.centroids[Number(clusters[i])]);
    }
    renderHotspots(cluster);
    for (var i = 0; i < 1623; i++) {
      for (var j = 0; j < clusters.length; j++) {
        if (i == clusters[j]) {
          if (materials['64'][i] != undefined) {
            materials['64'][i].opacity = 1;
          } else {
            console.log(i);
          }
          break;
