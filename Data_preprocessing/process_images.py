...
flags.DEFINE_integer('clusters', 1623, 'The number of clusters to display in the image browser')
...
class PixPlot:
  def __init__(self, image_glob):
    ...
    #self.load_image_vectors() # representative.npy
    #self.write_json()
  ...
  def load_image_vectors(self):
    ...
    for c, i in enumerate(self.vector_files):
      print(np.shape(np.load(i)))
      self.image_vectors = np.load(i)
      print(' * loaded', c+1, 'of', len(self.vector_files), 'image vectors')
    print(np.shape(self.image_vectors))
  ...
  def get_image_positions(self, fit_model):
    ...
    image_positions = []
    path = '/Users/jenny/Desktop/pix-plot-masterv2/utils/plot_data.json'
    self.images = [] 
    with open(path) as f:
      data = json.load(f)
    for item in range(len(data["centroids"])):
      self.images.append(data["centroids"][item]["img"])
  ...
  def get_centroids(self):
    ...
    '''
    path = '/Users/jenny/Desktop/pix-plot-masterv2/utils/coordinate.json' 
    centroid_json = []
    centroid = np.zeros((self.n_clusters,2))
    with open(path) as f:
      data = json.load(f)
    ...
    for i in range(self.n_clusters):
      x = centroid[i,0]/self.n_clusters
      y = centroid[i,1]/self.n_clusters
      dist = float("inf")
      img = []
      for item in range(len(data["nodes"])):
  ...
  def create_atlas_files(self):
    ...
    atlas_group_imgs = []
    print(self.sizes[1:])
    for thumb_size in self.sizes[0:]:
